...
if (isset($_GET['naik_status'])) {
    ...
    if ($new)
        pg_query($conn, "UPDATE transaksi SET status_order = '$new' WHERE id_transaksi = '$id'"); // ID kolom ganti ke id_transaksi dan dikutip
                ...
                <div class="card card-hover shadow-sm h-100 bg-white border-start border-4 border-primary">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div>
                            ...
                            <div class="h3 mb-0 fw-bold text-gray-800">Rp
                                <?= number_format($omset_hari_ini, 0, ',', '.') ?></div>
                        </div>
            ...
            <div class="col-md-4">
                <div class="card card-hover shadow-sm h-100 bg-white border-start border-4 border-warning">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        ...
                        <div class="fs-1 text-gray-300 text-warning opacity-25"><i
                        ...
                        <tbody>
                            ...
                            while ($r = pg_fetch_assoc($q)):
                                ...
                                <tr style="border-bottom: 1px solid #f0f0f0;">
                                    ...
                                    <td class="small text-muted"><?= date('d M y (H:i)', strtotime($r['waktu_order'])) ?>
                                    </td>
                                    ...
                                    <td class="fw-bold text-success">Rp <?= number_format($r['total_harga'], 0, ',', '.') ?>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <?php
                                        $st = $r['status_order'];
                                        // Update warna tombol: Proses(kuning) -> Selesai(biru/info) -> Done(hijau)
                                        $btn_c = ($st == 'Proses') ? 'warning' : 'info'; // Pakai info (biru) buat Selesai
                                    ...
                                    <td>
                                        <a href="edit_transaksi.php?id=<?= $r['id_transaksi'] ?>"
                                            class="btn btn-sm btn-light text-primary border rounded-circle" title="Edit"><i
                                                class="bi bi-pencil-fill"></i></a>
                                        <a href="index.php?hapus=<?= $r['id_transaksi'] ?>"
                                            onclick="return confirm('Hapus?')"
                                            class="btn btn-sm btn-light text-danger border rounded-circle" title="Hapus"><i
