...
<head>
    ...
    <style>
        ...
        .card-modern:hover .icon-box-stat {
...
<body>
    ...
    <div class="container pb-5 pt-0 mt-4">
        ...
        <div class="d-flex justify-content-between align-items-end mb-4 animate-fade-1">
            ...
            <div>
                <div class="bg-white px-3 py-2 rounded-3 border text-secondary small fw-medium shadow-sm"><i
                        class="bi bi-calendar-event me-2 text-primary"></i> <?= date('l, d F Y') ?></div>
            </div>
        </div>
            ...
            <div class="col-md-4">
                <div class="card-modern p-4 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h2 class="fw-bold m-0 text-dark"><?= $jumlah_proses ?> <span
                                class="fs-6 fw-normal text-secondary">Pesanan</span></h2>
                    </div>
            ...
            <div class="col-md-4">
                <div class="card-modern p-4 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h2 class="fw-bold m-0 text-dark"><?= $jumlah_order ?> <span
                                class="fs-6 fw-normal text-secondary">Pesanan</span></h2>
                    </div>
            ...
            <div class="col-md-4">
                <div class="card-modern p-4 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h2 class="fw-bold m-0 text-dark"><?= $jumlah_utang ?> <span
                                class="fs-6 fw-normal text-secondary">Data</span></h2>
                    </div>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                ...
                <div class="d-flex gap-2 w-90 w-md-auto justify-content-end align-items-center flex-wrap">
                    <form method="GET" class="d-flex gap-2 flex-grow-1 flex-md-grow-0 search-group">
                        <input type="date" name="tgl" class="form-control form-control-modern" style="width: auto;"
                            value="<?= $tanggal ?>" onchange="this.form.submit()">
                        <div class="position-relative flex-grow-1">
                            <input type="text" name="q" class="form-control form-control-modern ps-4"
                                placeholder="Cari ID / PO / Produk..." value="<?= $keyword ?>">
                            <i class="bi bi-search position-absolute top-50 start-0 translate-middle-y ms-2 text-secondary"
                                style="font-size: 0.8rem;"></i>
                        </div>
                    </form>

                    <button type="button" id="btnToggleCetak"
                        class="btn btn-dark d-flex align-items-center gap-2 shadow-sm px-3 py-2 btn-hover-effect"
                    ...
                    <a href="modules/transaksi/keranjang.php"
                        class="btn btn-modern d-flex align-items-center gap-2 shadow-sm px-4 py-2"
            ...
            <div class="card-modern overflow-hidden">
                <div id="toolbarCetak"
                    ...
                    <div class="d-flex align-items-center gap-2 text-warning-emphasis fw-bold"><i
                            class="bi bi-info-circle-fill"></i> <span>Mode Print Invoice: Centang satu ID, semua item
                            dengan ID sama otomatis terpilih.</span></div>
                ...
                <div class="table-responsive">
                    <table class="table table-custom mb-0 text-center">
                        <thead>
                            <tr>
                                <th class="ps-4 text-center col-checkbox" style="width: 50px; display: none;"><input
                                        type="checkbox" class="form-check-input" id="checkAll" style="cursor: pointer;">
                                </th>
                        ...
                        <tbody>
                                    ...
                                    <tr class="align-middle">
                                        ...
                                        <td class="text-start text-nowrap">
                                            ...
                                            <?php if (!empty($r['no_po'])): ?>
                                                <div class="small fw-bold text-dark mt-1" style="font-size: 0.8rem;">PO:
                                                    <?= $r['no_po'] ?></div>
                                            <?php endif; ?>
                                        ...
                                        <td class="text-start text-secondary" style="min-width: 150px; max-width: 250px;">
                                            <?= $r['nama_produk'] ?>
                                            <?php if ($r['panjang'] > 0): ?>
                                                <div class="small fst-italic text-muted mt-1" style="font-size: 0.75rem;"><i
                                                        class="bi bi-rulers"></i> <?= (float) $r['panjang'] ?>m x
                                                    <?= (float) $r['lebar'] ?>m</div>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td class="text-nowrap">
                                            ...
                                            <?php elseif ($st == 'Selesai'): ?>
                                                <a href="index.php?naik_status=true&uid=<?= $r['id'] ?>&status=<?= $st ?>"
                                                    class="badge badge-status bg-soft-info text-decoration-none">Siap Ambil <i
                                                        class="bi bi-chevron-right"></i></a>
                                            <?php else: ?>
                                                <a href="index.php?naik_status=true&uid=<?= $r['id'] ?>&status=<?= $st ?>"
                                                    class="badge badge-status bg-soft-warning text-decoration-none">Proses <i
                                                        class="bi bi-chevron-right"></i></a>
                                            <?php endif; ?>
                                        </td>
                                                ...
                                                <?php if ($r['status_pembayaran'] == 'Belum Lunas'): ?>
                                                    <div class="dropdown d-inline-block">
                                                        <button class="btn-icon btn-green dropdown-toggle" type="button"
                                                            data-bs-toggle="dropdown" style="border:none;" title="Pelunasan"><i
                                                                class="bi bi-check-lg"></i></button>
                                                        <ul class="dropdown-menu shadow-sm border-0">
                                                            <li><a class="dropdown-item"
                                                                    href="index.php?lunasi_item=true&uid=<?= $r['id'] ?>"
                                                                    onclick="return confirm('Lunasi ITEM ini saja?')">Lunasi Item
                                                                    Ini</a></li>
                                                            <li><a class="dropdown-item"
                                                                    href="index.php?lunasi_nota=true&id_trx=<?= $r['id_transaksi'] ?>"
                                                                    onclick="return confirm('Lunasi SEMUA di nota ini?')">Lunasi
                                                                    Satu Nota</a></li>
                                                        </ul>
                                                    </div>
                                                <?php endif; ?>
                                                <a href="modules/transaksi/edit.php?id=<?= $r['id_transaksi'] ?>"
                                                    class="btn-icon btn-blue" title="Edit"><i
                                                        class="bi bi-pencil-square"></i></a>
                                                <a href="modules/transaksi/invoice.php?item_id=<?= $r['id'] ?>"
                                                    class="btn-icon btn-gray" title="Print Item Ini"><i
                                                        class="bi bi-printer"></i></a>
                                                <a href="index.php?hapus=true&uid=<?= $r['id'] ?>"
                                                    onclick="return confirm('Hapus item ini?')" class="btn-icon btn-red"
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="9" class="text-center py-5 text-secondary"><i
                                            class="bi bi-receipt-cutoff fs-1 d-block mb-2 opacity-25"></i>Tidak ada data
                ...
                <nav>
                    <ul class="pagination shadow-sm bg-white rounded-pill p-1">
                        <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                            <a class="page-link" href="?page=<?= $page - 1 ?>&q=<?= $keyword ?>&tgl=<?= $tanggal ?>"><i
