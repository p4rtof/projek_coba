...
if (isset($_GET['naik_status'])) {
    ...
    if ($new)
        pg_query($conn, "UPDATE transaksi SET status_order = '$new' WHERE id_transaksi = '$id'");
    header("Location: index.php");
    exit();
}
...
<head>
    ...
    <style>
        ...
        /* Warna Hijau (Lunas) */
        .btn-green {
            background-color: #11cf579f;
        }

        .btn-green:hover {
            background-color: #1db956ff;
        }

        /* Warna Biru (Edit) */
        .btn-blue {
            background-color: #3b83f6b3;
        }
        ...
        <div class="card-modern mb-4 p-3">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                ...
                <div class="d-flex gap-2 w-95 w-md-auto justify-content-end align-items-center">
                    <form method="GET" class="d-flex gap-2 w-80 w-md-auto">
                        <div class="position-relative">
                            <input type="date" name="tgl" class="form-control form-control-modern" style="width: auto;"
                                value="<?= $tanggal ?>" title="Filter Tanggal" onchange="this.form.submit()">
                        </div>

                        <div class="position-relative flex-grow-1">
                            <input type="text" name="q" class="form-control form-control-modern"
                                placeholder="Cari Transaksi..." value="<?= $keyword ?>">
                        </div>

                        <?php if ($keyword || $tanggal): ?>
                            <a href="index.php"
                                class="btn btn-light border text-danger d-flex align-items-center justify-content-center"
                                style="width: 42px; border-radius: 10px;" title="Reset Filter">
                    ...
                    <div class="d-flex gap-2">
                        <a href="modules/transaksi/baru.php"
                                    ...
                                    <td class="text-end pe-4 text-nowrap">
                                        <div class="d-flex justify-content-end gap-2">
                                            ...
                                            <a href="modules/transaksi/edit.php?id=<?= $r['id_transaksi'] ?>"
                                                class="btn-icon btn-blue" title="Edit">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>

                                            <a href="modules/transaksi/invoice.php?id=<?= $r['id_transaksi'] ?>"
                                                class="btn-icon btn-gray" title="Print Invoice">
                                                <i class="bi bi-printer"></i>
                                            </a>

                                            <a href="index.php?hapus=<?= $r['id_transaksi'] ?>"
                                                onclick="return confirm('Hapus transaksi?')" class="btn-icon btn-red"
                                                title="Hapus">
                        ...
                        <ul class="pagination mb-0 me-2">
                            ...
                            for ($i = $start; $i <= $end; $i++): ?>
                                <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                    <a class="page-link"
                                        href="?page=<?= $i ?>&q=<?= $keyword ?>&tgl=<?= $tanggal ?>"><?= $i ?></a>
                                </li>
                            <?php endfor;
                            if ($end < $total_pages)
                                echo '<li class="page-item disabled"><span class="page-link">...</span></li>';
