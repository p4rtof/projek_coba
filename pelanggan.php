...
if (isset($_POST['simpan'])) {
    ...
    if ($_POST['id_edit']) {
        ...
        // 1. Ambil ID terakhir (misal P003)
        $q_last = pg_query($conn, "SELECT id_pelanggan FROM pelanggan ORDER BY id_pelanggan DESC LIMIT 1");
        $last_data = pg_fetch_assoc($q_last);

        // 2. Ambil angkanya saja, tambah 1
        $angka = $last_data ? (int) substr($last_data['id_pelanggan'], 1) : 0;
        $id_baru = 'P' . str_pad($angka + 1, 3, '0', STR_PAD_LEFT); // Jadi P004

        // 3. Masukkan ID baru ke query INSERT
