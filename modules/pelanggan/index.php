...
if (isset($_POST['simpan'])) {
    ...
    if ($_POST['id_edit']) {
        ...
        $angka = 0;
        if ($last_data) {
            // Hapus huruf 'P' di depan, sisa angka
            $angka = (int) substr($last_data['id_pelanggan'], 1);
        }

        // 3. Generate Angka Baru dengan Padding 4 Digit (Bisa sampai P9999)
        // Kalau mau sampai 100.000 user, ganti angka 4 jadi 5.
                        ...
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="hp" class="form-control"
                                    value="<?= $edit_data['hp'] ?? '' ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="small text-muted">Alamat</label>
                                <textarea name="alamat" class="form-control" rows="2"
                                    required><?= $edit_data['alamat'] ?? '' ?></textarea>
                            </div>
                    ...
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                        ...
                        <form method="GET" class="d-flex" style="width: 250px;">
                            <input type="text" name="q" class="form-control form-control-sm me-2"
                                placeholder="Cari nama..." value="<?= $_GET['q'] ?? '' ?>">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    ...
                                    // LOGIC QUERY SEARCH
                                    $keyword = $_GET['q'] ?? '';
                                    $q_tampil = "SELECT * FROM pelanggan WHERE nama ILIKE '%$keyword%' ORDER BY nama ASC";
                                    ...
                                    while ($row = pg_fetch_assoc($data_pelanggan)):
                                        ...
                                        <tr>
                                            ...
                                            <td><?= $row['alamat'] ?></td>
                                            <td>
                                                <a href="https://wa.me/<?= $row['hp'] ?>" target="_blank"
                                                    class="btn-whatsapp">
                                                    <i class="bi bi-whatsapp me-1"></i> <?= $row['hp'] ?>
                                                </a>
                                            </td>
                                            <td class="text-end pe-4">
                                                <a href="riwayat_pelanggan.php?id=<?= $row['id_pelanggan'] ?>"
                                                    class="btn btn-sm btn-info text-white me-1" title="Riwayat"><i
                                                        class="bi bi-clock-history"></i></a>
                                                <a href="pelanggan.php?edit=<?= $row['id_pelanggan'] ?>"
