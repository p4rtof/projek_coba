...
// --- 1. INISIALISASI DATA KE SESSION (Load data lama sekali saja) ---
if (!isset($_SESSION['edit_cart']) || $_SESSION['edit_id_trx'] != $id_transaksi) {
    ...
    // Ambil Detail Item
    $q_detail = pg_query($conn, "
        SELECT t.*, p.nama_produk, p.harga, p.jenis_satuan, p.stok_bahan 
        FROM transaksi t 
...
// --- 2. LOGIC TAMBAH ITEM (Masuk ke Session) ---
if (isset($_POST['tambah_item'])) {
    $id_prod = $_POST['id_produk'];
    $jumlah = (int)$_POST['jumlah'];
    
    // Ambil data produk dulu untuk cek jenis satuan
    $q_p = pg_query($conn, "SELECT * FROM produk WHERE id_produk = '$id_prod'");
    $prod = pg_fetch_assoc($q_p);

    // [FIX] Cek Jenis Satuan
    if ($prod['jenis_satuan'] == 'Meter') {
        ...
        // Jika bukan meter, paksa ukuran jadi 0
        $panjang = 0;
        $lebar = 0;
...
// --- 3. LOGIC HAPUS ITEM (Dari Session) ---
if (isset($_GET['hapus_idx'])) {
    $idx = $_GET['hapus_idx'];
    unset($_SESSION['edit_cart'][$idx]);
    $_SESSION['edit_cart'] = array_values($_SESSION['edit_cart']); 
    header("Location: edit.php?id=$id_transaksi");
    exit();
}

// --- 4. LOGIC SIMPAN PERUBAHAN (Final commit ke DB) ---
if (isset($_POST['simpan_perubahan'])) {
    // A. Kembalikan Stok Lama ke Produk
    ...
    // B. Hapus Semua Data Lama di Transaksi
    ...
    $status_byr = $_POST['status_pembayaran'];
    $status_ord = 'Proses'; 
    ...
    // D. Loop Simpan Item Baru dari Session
    foreach ($_SESSION['edit_cart'] as $item) {
        $id_p = $item['id_produk'];
        $qty = $item['jumlah'];
        $tot = $item['total_harga'];
        $p = $item['panjang'];
        $l = $item['lebar'];

        $q_ins = "INSERT INTO transaksi 
                  (id_transaksi, id_pelanggan, id_produk, waktu_order, jumlah, total_harga, status_pembayaran, status_order, panjang, lebar, metode_pembayaran, id_bank)
                  VALUES 
                  ('$id_transaksi', '$pelanggan_id', '$id_p', '$waktu_fix', $qty, $tot, '$status_byr', '$status_ord', $p, $l, '$metode', $id_bank)";
...
// Shortcut Variabel
$cart = $_SESSION['edit_cart'];
$head = $_SESSION['edit_header'];
?>
...
<body>
    ...
    <div class="container py-3">
        ...
        <div class="d-flex align-items-center mb-3">
            ...
            <div>
                <h5 class="fw-bold m-0 text-dark">Edit Transaksi <span class="text-primary">#<?= $id_transaksi ?></span></h5>
                <small class="text-secondary">Ubah item, tanggal, atau metode pembayaran</small>
            </div>
        </div>
                ...
                <div class="col-lg-5">
                    ...
                    <div class="card-modern mb-3">
                        <div class="header-gradient d-flex justify-content-between align-items-center">
                            <span class="fw-bold"><i class="bi bi-person me-2"></i>Data Utama</span>
                            <span class="badge bg-white bg-opacity-25 border border-white border-opacity-25">Edit Mode</span>
                        </div>
                    ...
                    <div class="card-modern">
                        <div class="p-3 border-bottom bg-light">
                            <h6 class="fw-bold m-0 text-primary"><i class="bi bi-cart-plus me-2"></i>Tambah / Edit Item</h6>
                        </div>
                        <div class="card-body p-3">
                            <div class="mb-3">
                                ...
                                <div class="row g-2">
                                    <div class="col-6">
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="number" step="0.01" name="panjang" id="input_p" class="form-control form-control-modern border-start-0 ps-1" value="1">
                                    ...
                                    <div class="col-6">
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="number" step="0.01" name="lebar" id="input_l" class="form-control form-control-modern border-start-0 ps-1" value="1">
                                            <span class="input-group-text bg-transparent border-0 text-secondary small">m</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row g-2 align-items-end">
                                <div class="col-4">
                                    ...
                                    <input type="number" name="jumlah" id="input_qty" class="form-control form-control-modern text-center fw-bold" value="1" min="1" oninput="validasiStok()"> 
                                </div>
                                <div class="col-8">
                                    <button type="submit" name="tambah_item" id="btn_tambah" class="btn btn-dark w-100" style="padding: 10px; border-radius: 8px; font-weight: 600;" formnovalidate>
                    ...
                    <div class="card-modern h-100 d-flex flex-column">
                        <div class="header-gradient bg-dark">
                            <div class="d-flex justify-content-between">
                                <span><i class="bi bi-basket me-2"></i> Rincian Order</span>
                                <span id="total_items_badge" class="badge bg-white text-primary"><?= count($cart) ?> Item</span>
                            </div>
                        </div>
                                ...
                                <tbody id="tabel_keranjang">
                                    ...
                                    <?php else: ?>
                                        ...
                                        foreach($cart as $idx => $item): 
                                            ...
                                            // LOGIC FIX: Cek Jenis Satuan untuk Tampilan Detail
                                            $detail = ($item['jenis_satuan'] == 'Meter') ? "Ukuran: {$item['panjang']}m x {$item['lebar']}m" : "-";
                                        ?>
                                        <tr>
                                            <td class="ps-4 fw-bold"><?= $item['nama_produk'] ?></td>
                                            ...
                                            <td class="text-center pe-3">
                                                <a href="edit.php?id=<?= $id_transaksi ?>&hapus_idx=<?= $idx ?>" class="btn btn-sm btn-light text-danger border"><i class="bi bi-trash"></i></a>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                        ...
                        <div class="p-3 bg-light border-top">
                            <div class="d-flex justify-content-between align-items-end mb-3 px-1">
                                <div class="text-secondary fw-bold small">Total Tagihan</div>
                                <h4 class="fw-bold text-success m-0" id="grand_total_display">Rp <?= number_format($grandTotal ?? 0, 0, ',', '.') ?></h4>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="row g-2">
                                    <div class="col-6">
                                        <input type="radio" class="radio-card-input" name="metode_pembayaran" id="metode1" value="Cash" onclick="cekMetode()" <?= $head['metode_pembayaran']=='Cash'?'checked':'' ?>>
                                        <label class="radio-card-label label-bank" for="metode1">
                                            <i class="bi bi-cash-coin radio-icon"></i> <div class="fw-bold small">Tunai</div>
                                        </label>
                                    </div>
                                    <div class="col-6">
                                        <input type="radio" class="radio-card-input" name="metode_pembayaran" id="metode2" value="Transfer" onclick="cekMetode()" <?= $head['metode_pembayaran']=='Transfer'?'checked':'' ?>>
                                        <label class="radio-card-label label-bank" for="metode2">
                                            <i class="bi bi-bank radio-icon"></i> <div class="fw-bold small">Transfer</div>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div id="area_bank" class="mt-2" style="display: none;">
                                    <div class="p-2 bg-white rounded-3 border">
                                        <label class="form-label small mb-2 text-secondary px-1">Pilih Rekening:</label>
                                        <div class="row g-2">
                                            <?php
                                            ...
                                            while ($b = pg_fetch_assoc($q_bank)) {
                                                ...
                                                ?>
                                                <div class="col-md-6">
                                                    <input type="radio" class="radio-card-input" name="bank_id" id="bank_<?= $b['id_bank'] ?>" value="<?= $b['id_bank'] ?>" <?= $sel ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="row g-2">
                                    <div class="col-6">
                                        <input type="radio" class="radio-card-input" name="status_pembayaran" id="status_lunas" value="Lunas" <?= $head['status_pembayaran']=='Lunas'?'checked':'' ?>>
                                    ...
                                    <div class="col-6">
                                        <input type="radio" class="radio-card-input" name="status_pembayaran" id="status_hutang" value="Belum Lunas" <?= $head['status_pembayaran']!='Lunas'?'checked':'' ?>>
                            ...
                            <button type="submit" name="simpan_perubahan" class="btn-modern py-3" onclick="return confirm('Yakin simpan perubahan ini? Data lama akan ditimpa.')">
        ...
        function cekProduk() {
            ...
            if (select.value !== "") {
                if (stok > 0) {
                    ...
                    areaInfo.innerHTML = `<span class="badge bg-danger bg-opacity-10 text-danger border border-danger"><i class="bi bi-x-circle me-1"></i> Stok Habis!</span>`;
                }
            } else {
                areaInfo.innerHTML = "";
            }
